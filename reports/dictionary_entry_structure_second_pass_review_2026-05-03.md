...
- `ko:n:기능`: `s3` now uses `entry_no: 2` with gloss `技能`; `s1` and `s2` keep `機能`.

The attached notes remain valid:

- `ko:n:지방` keeps `entry_no: 4` for `s3` as a pre-existing non-contiguous anomaly.
- `ko:n:예` keeps `例` to match the existing compatibility-ideograph encoding used elsewhere in the inventory.
