...
- `staging candidate` 提供可建置、可驗證的候選內容
...
| Staging Candidate Inventory | `content-pipeline/staging/**`、`staging/prototype_output/**` | staging 端候選內容池。用來建置、驗證、對照，不負責決定能不能發版。 | 否 |
| Production `manifest.json` | `lingo-frontend-web/assets/content/production/manifest.json` | production 端實際讀取的資產清單。它是產物，不是決策來源。 | 否，屬於輸出 |
| Production `lesson_catalog.json` | `lingo-frontend-web/assets/content/production/lesson_catalog.json` | production 端的課程中繼資料。包含單元、排序、標題、標籤等資訊。 | 否，屬於輸出 |
| Production bundles | `lingo-frontend-web/assets/content/production/packages/ko/**` | 實際被打包到 production 的 lesson JSON 與相關資產。 | 否，屬於輸出 |
...
這是實際被包進 production 的 lesson JSON 和相關資產。
